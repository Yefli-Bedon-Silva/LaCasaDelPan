...
public class ListarReclamosExcel extends AbstractXlsxView {
    ...
    protected void buildExcelDocument(Map<String, Object> model, Workbook workbook,
        ...
        response.setHeader("Content-Disposition", "attachment; filename=reclamos.xlsx");
        ...
        celdaTitulo.setCellValue("LISTADO DE RECLAMOS");
        ...
        String[] columnas = {"ID", "Cliente", "Motivo", "Detalle", "Estado"};
        ...
        List<Reclamo> reclamos = (List<Reclamo>) model.get("reclamos");
        int filaNum = 4;

        for (Reclamo reclamo : reclamos) {
            Row fila = sheet.createRow(filaNum++);
            fila.createCell(3).setCellValue(reclamo.getIdReclamo());
            fila.createCell(4).setCellValue(reclamo.getIdCliente().getNombreCli());
            fila.createCell(5).setCellValue(reclamo.getMotivoReclamo());
            fila.createCell(6).setCellValue(reclamo.getDetalle());
            fila.createCell(7).setCellValue(reclamo.getEstadoReclamo());
        }
